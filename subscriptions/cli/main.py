...
app = typer.Typer(name="subscriptions", help="Subscription analytics engine.")
...
def serve(
    ...
    uvicorn.run("subscriptions.api.app:app", host=host, port=port)


@app.command()
def worker() -> None:
    """Start the Kafka worker process."""
    from subscriptions.worker import run_worker
